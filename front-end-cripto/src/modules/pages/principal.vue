    ...
    <div class="mx-auto max-w-7xl px-2 sm:!px-6 lg:!px-8">
      <div class="relative flex h-16 items-center justify-between">
        <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
          <!-- Mobile menu button-->
          <DisclosureButton class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400  hover:text-white ">
            <span class="absolute -inset-0.5" />
            <span class="sr-only">Abrir menu de paginas</span>
        ...
        <div class="flex flex-1 items-center justify-center sm:items-stretch sm:!justify-start">
          ...
          <div class="hidden sm:ml-14 sm:block">
            <div class="flex space-x-4">
              <a v-for="item in navigation" :key="item.name" :href="item.href" download :class="[item.current ? 'bg-gray-800 text-white' : 'text-white hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</a>
            </div>
          </div>
        </div>
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                ...
                <MenuItem v-slot="{ active }">
                  <a href="#/login" :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">Inicio de sesión</a>
                </MenuItem>
              </MenuItems>
            </transition>
          </Menu>
...
const navigation = [
  ...
  { name: 'Mi llave publica', href: '/clave_publicaRS.der'},
